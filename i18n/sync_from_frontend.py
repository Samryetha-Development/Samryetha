...
用法：
  ...
  --check  只对比不写入（与 check_sync.py 等价：key 集合、顺序、值全比对），不一致则 exit 1。
"""
...
def compare_locale(
    ...
    与 check_sync.compare_locale 契约一致：
      - 缺 key / 多 key；
      - key 顺序与前端 json 不一致；
      - 每个 key 的 value 与前端不一致（值应照抄）。
    """
    problems: list[str] = []
...
def main() -> None:
    ...
    for locale in sorted(frontend):
        ...
        if args.check:
            ...
            seed_entries = data.get("entries", [])
            problems = compare_locale(locale, fkeys, seed_entries)
            if problems:
                print(f"[DIFF] {locale}: frontend={len(fkeys)} seed={len(seed_entries)}")
                for p in problems:
                    print(f"  {p}")
                failed = True
            else:
                print(f"[OK] {locale}: {len(fkeys)} keys aligned (order + values)")
            continue
